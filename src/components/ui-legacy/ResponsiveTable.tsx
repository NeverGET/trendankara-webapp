    ...
    React.Children.forEach(children, (child) => {
      if (React.isValidElement(child)) {
        if (child.type === ResponsiveTableHeader) {
          // Extract headers
          React.Children.forEach((child as any).props.children, (headerRow) => {
            if (React.isValidElement(headerRow)) {
              React.Children.forEach((headerRow as any).props.children, (headerCell) => {
                if (React.isValidElement(headerCell)) {
                  const headerText = typeof (headerCell as any).props.children === 'string'
                    ? (headerCell as any).props.children
                    : 'Column';
          ...
          // Extract rows
          React.Children.forEach((child as any).props.children, (row, rowIndex) => {
            if (React.isValidElement(row)) {
              const cells: React.ReactNode[] = [];
              let rowProps = { clickable: false, onClick: undefined };

              if ((row as any).props.clickable) rowProps.clickable = true;
              if ((row as any).props.onClick) rowProps.onClick = (row as any).props.onClick;

              React.Children.forEach((row as any).props.children, (cell) => {
                if (React.isValidElement(cell)) {
                  cells.push((cell as any).props.children);
                }
              });
